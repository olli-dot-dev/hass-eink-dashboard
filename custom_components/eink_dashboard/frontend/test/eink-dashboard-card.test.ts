...
describe("applyEdgeResize", () => {
  ...
  it("w: dragging right narrows widget and moves x forward", () => {
    // dx=16 shifts left edge right: startRight=224, newX=snap(40)=40, w=snap(184)=184.
...
describe("applyCornerResize", () => {
  ...
  it("se: expanding drag increases width and height", () => {
    const r = applyCornerResize("se", 16, 16, 24, 0, 200, 100, 20, DW, DH);
    expect(r.x).toBeUndefined();
  ...
  it("se: clamps to minDim on inward drag", () => {
    // Shrinking past minimum clamps to minDim.
    const r = applyCornerResize("se", -9999, -9999, 24, 0, 200, 100, 20, DW, DH);
    expect(r.w).toBe(snap(20));
    expect(r.h).toBe(snap(20));
  });

  it("se: clamps width at display right boundary", () => {
    // Widget at x=24 cannot grow past displayWidth=800; max w=snap(776)=776.
    const r = applyCornerResize("se", 9999, 0, 24, 0, 200, 100, 20, DW, DH);
    expect(r.w).toBe(snap(DW - 24));
  });

  it("se: clamps height at display bottom boundary", () => {
    // Widget at y=0 cannot grow past displayHeight=600.
    const r = applyCornerResize("se", 0, 9999, 24, 0, 200, 100, 20, DW, DH);
  ...
  it("ne: dragging up (dy<0) moves top edge up and grows height", () => {
    ...
    // h = 140-24 = 116.
    const r = applyCornerResize("ne", 16, -16, 24, 40, 200, 100, 20, DW, DH);
    expect(r.x).toBeUndefined();
    expect(r.y).toBe(24);
    expect(r.w).toBe(216);
    expect(r.h).toBe(116);
  ...
  it("sw: dragging left (dx<0) expands width and moves x left", () => {
    // newX=snap(max(0,min(204,24-16)))=snap(8)=8; w=startRight(224)-8=216.
    const r = applyCornerResize("sw", -16, 16, 24, 40, 200, 100, 20, DW, DH);
    expect(r.y).toBeUndefined();
    expect(r.x).toBe(8);
    expect(r.w).toBe(216);
    expect(r.h).toBe(120);
  ...
  it("sw: clamps x at 0", () => {
    ...
    const r = applyCornerResize("sw", -9999, 0, 24, 40, 200, 100, 20, DW, DH);
    expect(r.x).toBe(0);
  });
  ...
  it("nw: dragging inward shrinks width and height, moves x and y", () => {
    ...
    const r = applyCornerResize("nw", 16, 16, 24, 40, 200, 100, 20, DW, DH);
    expect(r.x).toBe(40);
    expect(r.y).toBe(56);
    expect(r.w).toBe(184);
    expect(r.h).toBe(84);
